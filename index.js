...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Hallo @${num.split('@')[0]}\Selamat datang di group *${mdata.subject}* yang betah ya di sini (｡•̀ᴗ-)✧`
				...
				teks = `akhirnya beban group berkurang 𝟭,bye bye 🥳 @${num.split('@')[0]} jasamu akan di kubur dalam² ಥ_ಥ`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6285726125902@s.whatsapp.net"] 
        ...
        /*        case 'kickme':
          *      if (isGroupAdmins) return reply(`maaf ${pushname} saya tidak bisa mengeluarkan admin`)
           ...
           *     await client.groupRemove(groupId, sender)
                ...
                if (!isRegistered) return reply(ind.noregis())
                if (getLevelingLevel(sender) < ahc ) return reply(ind.reglevelahc(command, pushname, getLevelingLevel, sender, ahc))
              	    if (args.length < 1) return reply('teksnya mana hyung?')
                    teks = `${body.slice(8)}`
                    if (teks.length > 10) return client.sendMessage(from, 'Teksnya kepanjangan hyung, Maksimal 10 kalimat (｡•̀ᴗ-)✧', text, {quoted: mek})
			...
			if (getLevelingLevel(sender) < ahd ) return reply(ind.reglevelahd(command, pushname, getLevelingLevel, sender, ahd))
				...
				if (getLevelingLevel(sender) < ahb ) return reply(ind.reglevelahb(command, pushname, getLevelingLevel, sender, ahb))
					try {
						...
						reply('[𝗘𝗥𝗥𝗢𝗥] 𝗸𝗲𝗺𝘂𝗻𝗴𝗸𝗶𝗻𝗮𝗻 𝘂𝘀𝗲𝗿𝗻𝗮𝗺𝗲 𝘁𝗶𝗱𝗮𝗸 𝘃𝗮𝗹𝗶𝗱 (´-﹏-`；)')
					}
					break
                 case 'linkgc':
				    if (!isGroup) return reply(ind.groupo())
